...
namespace Varia.Items.FallenAngel
{
	...
	public class MegaCannonProj : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			if (AtMaxCharge)
			{
				DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center,
					projectile.velocity, 10, projectile.damage, -1.57f, 1f, 1000f, new Color(Main.DiscoR / 3, 0, 58), (int)MoveDistance);
			}
			return false;
		}
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			{
				Color c = new Color(Main.DiscoR / 3, 0, 58);
				origin = start + i * unit;
			...
			spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
				new Rectangle(0, 0, 28, 26), new Color(Main.DiscoR / 3, 0, 58), r, new Vector2(28 * .5f, 26 * .5f), scale, 0, 0);
			#endregion

			#region Draw laser head
			spriteBatch.Draw(texture, start + (Distance + step) * unit - Main.screenPosition,
				new Rectangle(0, 52, 28, 26), new Color(Main.DiscoR / 3, 0, 58), r, new Vector2(28 * .5f, 26 * .5f), scale, 0, 0);
			#endregion
		}
		...
		public override void AI()
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, 226, dustVel.X, dustVel.Y, 0, new Color(Main.DiscoR / 3, 0, 58), 1f)];
				dust.noGravity = true;
				dust.scale = 1.2f;
				...
				dust.noGravity = true;
				dust.scale = 0.88f;
				dust.color = new Color(Main.DiscoR / 3, 0, 58);
			}
			if (Main.rand.Next(5) == 0)
			{
				Vector2 offset = projectile.velocity.RotatedBy(1.57f, new Vector2()) * ((float)Main.rand.NextDouble() - 0.5f) *
								 projectile.width;
				Dust dust = Main.dust[
					Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, 31, 0.0f, 0.0f, 100, new Color(Main.DiscoR / 3, 0, 58), 1.5f)];
				...
				unit.Normalize();
				dust = Main.dust[
					Dust.NewDust(Main.player[projectile.owner].Center + 55 * unit, 8, 8, 31, 0.0f, 0.0f, 100, new Color(Main.DiscoR / 3, 0, 58), 1.5f)];
				dust.velocity = dust.velocity * 0.5f;
				dust.velocity.Y = -Math.Abs(dust.velocity.Y);
			}
			#endregion
