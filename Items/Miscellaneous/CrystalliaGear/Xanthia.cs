...
namespace Varia.Items.Miscellaneous.CrystalliaGear
{
	public class Xanthia : ModItem
	{
        ...
        {
			for (int i = 0; i < 10; i++) Projectile.NewProjectile(player.Center.X,  player.Center.Y,  speedX + Main.rand.Next(-10,  10),  speedY + Main.rand.Next(-10,  10),  90,  damage - Main.rand.Next(1,  27),  knockBack,  player.whoAmI,  0.0f,  0.0f);
            return false;
		}
	}
}
